...
class Chat(AuthorizationMixin,APIView):
    def get(self,request:HttpRequest,user_id):
        try:
            messages=execute("select messages.id,sender_id,text,users,username from messages where users.id=sender_id  (%s,%s) in ((sender_id,reciever_id) ,(reciever_id,sender_id) ) order by time asc ",[request.COOKIES["id"],user_id])
